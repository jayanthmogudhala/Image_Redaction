...
pytesseract.pytesseract.tesseract_cmd = "/usr/bin/tesseract"  # Default path for Linux
...
def redact_regions(image_path, faces, sensitive_text_boxes, output_path, blur_radius=15):
    ...
    for (x, y, w, h) in faces:
        redaction_boundary = (x, y, x + w, y + h)
...
if uploaded_file is not None:
    ...
    blur_percentage = st.slider("Select Blur Percentage", 0, 100, 50)
    blur_radius = blur_percentage * 0.2
    ...
    if st.button("Redact Image"):
        ...
        redacted_image_path = redact_regions(file_path, faces, sensitive_text_boxes, output_path, blur_radius)
